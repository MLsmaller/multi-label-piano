...
from test_binary import test_videos
...
if opt.with_Time is None:
    ...
    train_set = TimeDataset_select(train_path, img_size=cfg.octave_final_size
                                , k=k)
    val_set = TimeDataset_select(val_path, img_size=cfg.octave_final_size
                                , phase='val', k=k)
...
label_weight = torch.Tensor(cfg.octave_label_weights).double().to(device)
...
def eval_video(model):  
    ...
    for video_path in test_img_lists:
        ...
        begin=time.time()
        test_videos(model, hand_seg, VisAmt, video_path)
        end = time.time()
...
def train(model):
    ...
    param = 'lr: {}\tprob_thresh: {}\ttop_k: {}\tlr_decay_in_epoch: {}\n'.format(
            opt.lr, cfg.octave_prob_thresh, cfg.octave_top_k, opt.lr_decay_in_epoch)
    ...
    if opt.with_Time:
        if opt.Data_type:
            #--包含位置和时间信息
            torch.save(best_model_wts, "checkpoints/octave_time_with_pos_keys_epoch_{}_Acc_{:.3f}.pth".format(best_epoch, best_acc))
        else:
            #--只包含时间信息
            torch.save(best_model_wts, "checkpoints/octave_time_keys_epoch_{}_Acc_{:.3f}.pth".format(best_epoch, best_acc))
    elif opt.Data_type:
        #---包含位置信息不包含时间信息
        torch.save(best_model_wts, "checkpoints/octave_with_pos_keys_epoch_{}_Acc_{:.3f}.pth".format(best_epoch,best_acc))   
    else:
        #---什么都不包含
        torch.save(best_model_wts, "checkpoints/octave_keys_epoch_{}_Acc_{:.3f}.pth".format(best_epoch,best_acc))   
