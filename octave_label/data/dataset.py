...
class KeyDataset_select(Dataset):
    def __init__(self, list_path, img_size=(224, 165),phase='train'):
        ...
        #--[0,1,2,4,5,6,7,9,11]         #--3/8/10按键数量较少
...
class TimeDataset_select(Dataset):
    def __init__(self, list_path, img_size=(224, 165),k=5,phase='train'):
        self.phase=phase
        self.k = k  #---输入连续k帧
...
if __name__ == '__main__':
    val_set = TimeDataset_select(cfg.octave_val_list, img_size=cfg.octave_final_size,
                                 k=5, phase='val')
    # train_set = TimeDataset_select(cfg.octave_train_list, img_size=cfg.octave_final_size,
    #                              k=5, phase='train')
    img_paths, img_lists, targets = val_set.__getitem__(2)
    #---加了batch_size之后返回值怪怪的，可以看一下
    ...
    # for batch_i ,(img_paths, img_lists, targets) in enumerate(val_dataloader):
    #     print(batch_i)
    #     for img in img_lists:
    #         print(img.shape)
        

    # train_set = KeyDataset_select(cfg.octave_train_list, img_size=cfg.octave_final_size)
