...
#---将八度中的键转换为1-88的按键
def convert2note(idx, key_indexs, mode):
    new_indexs = []
    for index in key_indexs:
        index = cfg.octave_w[index] if mode == 'white' else cfg.octave_b[index]
...
def test_videos(model, hand_seg, VisAmt, video_path,opt):
    ...
    detectPath = os.path.join(cfg.Test_Key_Dir, file_seq, str(random_num) + txt_path)
    w_detectPath = os.path.join(cfg.Test_Key_Dir, file_seq, str(random_num) + octave_w_path)
    b_detectPath = os.path.join(cfg.Test_Key_Dir, file_seq, str(random_num) + octave_b_path)
    ...
    for path in tqdm(img_list):
        ...
        for ((img_path, idx), img) in test_lists:
            ...
            #---白键和黑键转换过去还不一样
            new_indexs = convert2note(idx, key_indexs, opt.mode)
    ...
    #w_F是白键的帧级的准确率, notew_F是白键的onset的准确率
    return evaluate.w_F, evaluate.notew_F  
...
def main(img_lists, test_img_lists):
    ...
    if opt.with_Time:
        ...
        ckpt_path = cfg.wboctave_time_path if opt.Data_type is None else cfg.time_pos_ckpt_path
    else:
