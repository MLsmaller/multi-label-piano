...
def near_octave_white(white_loc, boxes):
    index_list = []
    if len(boxes)==0:
        return index_list
    ...
    octave_idx = []
    for i in range(7):
        octave_idx.append(2 + i * 7)
    for idx, loc in enumerate(white_loc):
        if idx in octave_idx:
            new_loc.append((white_loc[idx], white_loc[idx + 7]))
    new_loc = sorted(new_loc, key=lambda x: x[0])
    

    for box in boxes:
        min_loc = box[0][0]
        max_loc = box[1][0]
        for idx, w_loc in enumerate(new_loc):
            #---顺序查找找到第一个小于loc的前一个octave
            if min_loc - w_loc[0] < 0:
                left_index = idx - 1
                break
            left_index = 6
        ...
        if left_index == 6:
            ...
            right_index = left_index + 1 if max_loc >new_loc[left_index][1] else left_index
...
def vis_white_black_loc_youtube(white_loc, boxes, save_path, img_lists, rect, base_info):
    ...
    for img_file in img_lists:
        ...
        w,h = img.size 
        file_seq = os.path.dirname(img_file).split('/')[-1]
        ...
        if base_info is not None:
            if base_info['warp_M'] is not None:
                ...
                ori_img = Image.fromarray(cv2.cvtColor(warp_img,cv2.COLOR_BGR2RGB))
                ...
                ori_img = Image.fromarray(cv2.cvtColor(rotated_img,cv2.COLOR_BGR2RGB))
            else:
