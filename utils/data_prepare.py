...
def get_train_img(img_path):
    ...
    if file_seq in ['level_2_no_01','level_4_no_01']:
        w_offset=4
        b_offset=3
        #---即黑键的下边界延长一点
        y2_offset=2
...
def from_train_data(img_paths):
    ...
    for img_path in img_paths:
        ...
        #---没有midi_offset这个key值的是新加的视频，不是数据集，用以测试
        if not 'midi_offset' in cfg.EVALUATE_MAP[os.path.basename(img_path)]: continue
        print(img_path)
